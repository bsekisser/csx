...
static int dskimg_dump = 0;
const int dskimg_load = 1;
const int dskimg_write = 1;

typedef char page_t[(512 << 2) + (16 << 2)]; // ec,d3
...
static uint32_t csx_nnd_flash_read(csx_nnd_ref nnd, const uint32_t addr, const size_t size);
static void csx_nnd_flash_write(csx_nnd_ref nnd, const uint32_t addr, const size_t size, const uint32_t value);
...
static const uint8_t csx_nnd_flash_id[16][8] = {
//	[0xc] = { 0xec, 0x76, 0x51, 0x95, 0x58 }, // ???
//	[0x8] = { 0xec, 0x75, 0x51, 0x95, 0x58 },
//	[0xc] = { 0xec, 0x76, 0x51, 0x95, 0x58 },
...
if(1)		LOG("row: 0x%08x, block: 0x%06x, 0x%016" PRIxPTR ", 0x%016" PRIxPTR,
...
if(1)		LOG("row: 0x%08x, block: 0x%06x, page: 0x%02x, 0x%016" PRIxPTR,
	...
	const uint32_t row)
{
	...
	void *const p = __csx_nnd_flash_p2page(nnd, unit, row, (void*)0xfeedface);
	assert(p);
...
if(1) 	LOG("row: 0x%08x --> block: 0x%06x, page: 0x%03x", row, block, page);
	...
	if(dskimg_dump)
		dump_hex(p, row, sizeof(page_t), 16, 1);
}
...
void csx_nnd_flash_write(csx_nnd_ref nnd, const uint32_t addr, const size_t size, const uint32_t value)
{
	...
	switch(offset) {
		case ALE:
			if(1) LOG("ALE: cs: 0x%01X, cl = 0x%08x, value = 0x%08x, size = 0x%02zx, row = 0x%06x, column = 0x%03x",
				cs, unit->cl, value, size, unit->row, unit->column);
			return(csx_nnd_flash_write_ale(nnd, unit, size, value));
		case CLE:
			if(1) LOG("CLE: cs: 0x%01X, cl = 0x%08x, value = 0x%08x, size = 0x%02zx, row = 0x%06x, column = 0x%03x",
				cs, unit->cl, value, size, unit->row, unit->column);
			return(csx_nnd_flash_write_cle(nnd, unit, size, value));
		case RWD:
			if(1) LOG("RWD: cs: 0x%01X, cl = 0x%08x, value = 0x%08x, size = 0x%02zx, row = 0x%06x, column = 0x%03x",
				cs, unit->cl, value, size, unit->row, unit->column);
