...
uint32_t csx_mmio_watchdog_read(csx_mmio_watchdog_p wdt, uint32_t addr, uint8_t size)
{
	csx_p csx = wdt->csx;
...
void csx_mmio_watchdog_write(csx_mmio_watchdog_p wdt, uint32_t addr, uint32_t value, uint8_t size)
{
	csx_p csx = wdt->csx;
