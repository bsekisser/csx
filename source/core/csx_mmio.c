...
#define MMIO_LIST \
	...
	MMIO(0xfffe, 0x1140, 0x0000, 0x0000, 0, MEM_TRACE_RW, x0xfffe_0x1140) \
	MMIO(0xfffe, 0x1160, 0x0000, 0x0000, 0, MEM_TRACE_RW, x0xfffe_0x1160) \
	\
...
#undef MMIO
#define MMIO(_ahi, _alo, _dhi, _dlo, _size, _access, _name) \
	{ ((_ahi ## ULL << 16) | _alo ## ULL), ((_dhi ## ULL << 16) | _dlo ## ULL), _size, _access, #_name, },
...
{
	csx_mmio_p mmio = csx->mmio;
	ea_trace_p eat = omap_mmio_get_trace(vaddr);
	const char *name = eat ? eat->name : "";
	
	uint32_t paddr = vaddr - CSX_MMIO_BASE;
	
	LOG("cycle = 0x%016llx, vaddr = 0x%08x, paddr = 0x%08x, name = %s", csx->cycle, vaddr, paddr, name);
	
	if(vaddr >= CSX_MMIO_BASE && vaddr <= CSX_MMIO_STOP)
	{
		uint32_t value = csx_data_read(&mmio->data[paddr], size);

		switch(vaddr)
		{
			case MPU_READ_TIM_3:
				value = csx->cycle - mmio->timer_base[3];
				csx_data_write(&mmio->data[paddr], value, size);
				break;
		}
		return(value);
	}
...
void csx_mmio_write(csx_p csx, uint32_t vaddr, uint32_t value, uint8_t size)
{
	csx_mmio_p mmio = csx->mmio;
	ea_trace_p eat = omap_mmio_get_trace(vaddr);
	const char* name = eat ? eat->name :  "";
	...
	LOG("cycle = 0x%016llx, vaddr = 0x%08x, paddr = 0x%08x, value = 0x%08x, name = %s", csx->cycle, vaddr, paddr, value, name);
	...
	if(vaddr >= CSX_MMIO_BASE && vaddr <= CSX_MMIO_STOP)
	{
		switch(vaddr)
		{
			...
			case MPU_LOAD_TIM_3:
				...
				csx_mmio_write(csx, MPU_READ_TIM_3, value, size);
				break;
		}
		csx_data_write(&mmio->data[paddr], value, size);
	}
...
int csx_mmio_init(csx_p csx)
{
	...
	do {
		...
		csx_data_write(&mmio->data[paddr], te->reset_value, te->size >> 3);
//		csx_mmio_write(csx, te->address, te->reset_value, te->size >> 3);
	}while(trace_list[i].address);
