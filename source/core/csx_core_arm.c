...
{
	const int blx = (0x0f == BFEXT(opcode, 31, 28));

	int link = BEXT(opcode, ARM_INST_BIT_LINK);
	const int32_t offset = BFSEXT(opcode, 23, 0);
...
//		ls.rd_v &= _BM((ls.rw_size << 3) - 1);
		...
		if(ls.flags.s) /* sign extend ? */
			ls.rd_v = BFSEXT(ls.rd_v, (8 << (ls.rw_size >> 1)), 0);
	}
	else
...
//	}bit;
	...
	const uint8_t field_mask = BFEXT(opcode, 19, 16);
...
{
	const uint8_t cc = BFEXT(opcode, 31, 28);
...
//check1:
	switch(check)	/* check 1 */
	{
		...
		case _INST1(1): /* xxxx 001x xxxx xxxx */
			if((_BF(25, 24) | _BV(21)) == (ir & (_BF(27, 23) | _BF(21, 20))))
				;
