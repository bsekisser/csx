...
void csx_trace_psr(csx_core_p core, const char* pfn, uint32_t psr)
{
	...
	dst += snprintf(dst, end - dst, ":0x%01x:", _MLBFX(psr, 26, 25));
	dst += snprintf(dst, end - dst, ":%c:", BEXT(psr, 24) ? 'J': 'j');
	dst += snprintf(dst, end - dst, ":0x%01x:", _MLBFX(psr, 23, 20));
	dst += snprintf(dst, end - dst, ":GE[19:16] = %1x:", _MLBFX(psr, 19, 16) & 0x0f);
	dst += snprintf(dst, end - dst, ":0x%02x:", _MLBFX(psr, 15, 10));
	...
	const uint8_t mode = _MLBFX(psr, 4, 0);
