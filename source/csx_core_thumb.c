...
static void csx_core_thumb_add_sub_rn_rd(csx_core_p core)
{
	...
	csx_core_decode_get(core, rRM, 8, 6, !bit_i);
	if(bit_i)
		vR(M) = rR(M);

	csx_core_decode_get(core, rRN, 5, 3, 1);
	csx_core_decode_get(core, rRD, 2, 0, 0);
	...
	if(op2)
		csx_core_flags_nzcv_sub(core, vR(D), vR(N), vR(M));
	else
		csx_core_flags_nzcv_add(core, vR(D), vR(N), vR(M));
}
...
static void csx_core_thumb_add_rd_pcsp_i(csx_core_p core)
{
	...
	csx_core_decode_get(core, rRD, 10, 8, 0);
	...
	uint32_t res = vR(D) + imm8;

//	LOG("rPC:rSP");
	CORE_TRACE("add(%s, %s, 0x%03x); /* 0x%08x + 0x%03x = 0x%08x */",
				_arm_reg_name(rR(D)), pcsp ? "rSP" : "rPC", imm8, vR(D), imm8, res);
...
static void csx_core_thumb_ascm_rd_i(csx_core_p core)
{
	...
	csx_core_decode_get(core, rRD, 10, 8, 1);
	...
	{
		case THUMB_ASCM_OP_ADD:
			csx_core_flags_nzcv_add(core, res, vR(D), imm8);
			break;
		case THUMB_ASCM_OP_CMP:
		case THUMB_ASCM_OP_SUB:
			csx_core_flags_nzcv_sub(core, res, vR(D), imm8);
			break;
		case THUMB_ASCM_OP_MOV:
...
static void csx_core_thumb_bx(csx_core_p core)
{
	...
	csx_core_decode_get(core, rRM, 6, 3, 1);
...
static void csx_core_thumb_dp_rms_rdn(csx_core_p core)
{
	...
	csx_core_decode_get(core, rRM, 5, 3, 1);
...
static void csx_core_thumb_ldst_rd_i(csx_core_p core)
{
	...
	if(bit_l)
		vR(D) = csx_core_read(core, vR(N), sizeof(uint32_t));
	else
	...
	if(bit_l)
		...
		csx_core_write(core, rR(N), vR(D), sizeof(uint32_t));
}
...
//	}bit;
	...
	if(bit_l)
		vR(D) = csx_core_read(core, ea, size);
	else
		vR(D) = csx_reg_get(core, rR(D));
	...
	if(bit_l)
		...
		csx_core_write(core, ea, vR(D), size);
}
...
//	}bit;
	...
	csx_core_decode_get(core, rRM, 8, 6, 1);
	csx_core_decode_get(core, rRN, 5, 3, 1);
	csx_core_decode_get(core, rRD, 2, 0, 0);
...
//	}bit;
	...
	csx_core_decode_get(core, rRN, 10, 8, 1);
