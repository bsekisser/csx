...
UNUSED_FN static uint32_t soc_mmio_mpu_mmc_read(void* param, void* data, uint32_t addr, size_t size)
{
	const soc_mmio_mpu_mmc_p mmc = param;
	const csx_p csx = mmc->csx;

	uint32_t value = csx_data_read_offset(data, (addr & 0xff), size);;
...
UNUSED_FN static void soc_mmio_mpu_mmc_write(void* param, void* data, uint32_t addr, size_t size, uint32_t value)
{
	...
	if(eat)
	{
		...
		csx_data_write_offset(data, (addr & 0xff), size, value);
	} else {
...
int soc_mmio_mpu_mmc_init(csx_p csx, soc_mmio_p mmio, soc_mmio_mpu_mmc_h h2mmc)
{
	...
	soc_mmio_mpu_mmc_p mmc = HANDLE_CALLOC(h2mmc, 1, sizeof(soc_mmio_mpu_mmc_t));
	ERR_NULL(mmc);
